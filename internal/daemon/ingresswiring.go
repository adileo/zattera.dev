...
func startDevIngress(ctx context.Context, cfg configForIngress, rb *scheduler.RouteBuilder, authority *ca.CA, nodeID string, clk clock.Clock, statsSink func(*proxy.Stats), activate proxy.ActivateFunc, log *slog.Logger) error {
	tm, err := tlsmgr.New(tlsmgr.Options{Dev: true, CA: authority, Logger: log})
	if err != nil {
		return err
	}
	return serveIngress(ctx, cfg, routeBuilderSource{rb: rb}, tm, true, nodeID, clk, statsSink, activate, log)
}
...
func startProdIngress(ctx context.Context, cfg config.Config, source proxy.RouteSource, tm *tlsmgr.Manager, nodeID string, clk clock.Clock, statsSink func(*proxy.Stats), activate proxy.ActivateFunc, log *slog.Logger) error {
	...
	return serveIngress(ctx, ic, source, tm, false, nodeID, clk, statsSink, activate, log)
}
...
func serveIngress(ctx context.Context, cfg configForIngress, source proxy.RouteSource, tm *tlsmgr.Manager, disableRedirect bool, nodeID string, clk clock.Clock, statsSink func(*proxy.Stats), activate proxy.ActivateFunc, log *slog.Logger) error {
	l7 := proxy.NewL7(source, nodeID, clk)
	l7.DisableHTTPSRedirect = disableRedirect
	// Wake-on-request for scaled-to-zero envs (T-70).
	if activate != nil {
		l7.SetActivator(proxy.NewActivator(source, activate, clk))
	}
